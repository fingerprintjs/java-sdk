...
public class Sealed {
  ...
  private static final byte[] SEAL_HEADER =
      new byte[] {(byte) 0x9E, (byte) 0x85, (byte) 0xDC, (byte) 0xED};
  private static final int NONCE_LENGTH = 12;
  private static final int AUTH_TAG_LENGTH = 16;

  public static byte[] unseal(byte[] sealed, DecryptionKey[] keys)
      throws IllegalArgumentException, UnsealAggregateException {
        ...
        case AES_256_GCM:
          try {
            return decryptAes256Gcm(
                Arrays.copyOfRange(sealed, SEAL_HEADER.length, sealed.length), key.key);
          } catch (Exception exception) {
            aggregateException.addSuppressed(
                new UnsealException("Failed to decrypt", exception, key));
          }
   ...
   * @return the {@link Event} stored in the sealed data
   *
   ...
   */
  public static Event unsealEventResponse(byte[] sealed, DecryptionKey[] keys)
      throws IllegalArgumentException, UnsealAggregateException, IOException {
