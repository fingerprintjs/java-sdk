...
public class FingerprintApi {
  public static final String INTEGRATION_INFO = "fingerprint-pro-server-java-sdk/8.7.0";
  ...
  // Return type is now the Event interface (EventDevice | EventEdge).
  // response.getIdentification() no longer compiles. Identification is on EventDevice.
  ...
  public static class SearchEventsOptionalParams {
    ...
    /**
     * getter for end - Include events that happened before this point (with timestamp less than or equal to the provided `end` Unix milliseconds value). Defaults to now. Setting `end` does not change the default of `7 days ago` for `start`/`start_date_time` — adjust it separately if needed.
     *
    ...
    /**
     * setter for end - Include events that happened before this point (with timestamp less than or equal to the provided `end` Unix milliseconds value). Defaults to now. Setting `end` does not change the default of `7 days ago` for `start`/`start_date_time` — adjust it separately if needed.
     *
